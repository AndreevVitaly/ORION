...
def _split_issues(value: str | None) -> list[str]:
    if not value:
        return []
...
def _status_index(rows: list[dict]) -> dict[str, dict]:
    ...
    for row in rows:
        ...
        if report_name:
            index[report_name] = row
        if image_name:
            index[image_name] = row
    return index
...
def _measurement_summary(reports: list[dict]) -> dict:
    ...
    for name, path in selected.items():
        values = [
            float(value)
            for report in reports
            if (value := _measurement_value(report, path)) is not None
        ]
...
def _candidate_brief(report: dict) -> dict:
    lic_core = report.get("lic_core") or {}
...
def build_report_pack(report_directory: str, include_frames: bool = True, top: int = 10) -> dict:
    """Собрать компактный исследовательский пакет из папки portrait JSON."""
...
def render_markdown(pack: dict) -> str:
    ...
    lines.extend([
        "",
        "## LIC",
        f"Лучший кандидат: `{lic.get('best_candidate')}`",
    ])
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Упаковать серию portrait JSON в компактный исследовательский отчет"
    )
